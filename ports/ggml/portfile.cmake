...
# pre-applied, plus the Wan-required Metal ops + the Supertonic 2
# fused custom op family.  Pinned to the head of the `speech` branch.
#
# Pinned to 60a172e48f -- the merge commit of tetherto/qvac-ext-ggml#8.
# On top of the previous pin (05afdc59, the merge of #6) this brings:
#
#   - Vulkan / Metal / CMake fixes cherry-picked from the 2026-01-30
#     branch (persistent VkPipelineCache + crash-safe eager flush,
#     GGML_LIB_OUTPUT_PREFIX option, hybrid backend packaging,
#     GGML_METAL_FUSE_MV_BIAS CMake option, std::filesystem
#     pipeline-cache rename).  Merged via qvac-ext-ggml#7.
#   - The Supertonic 2 fused custom op family (qvac-ext-ggml#8): five
#     fused Metal kernels (depthwise_1d / layer_norm_channel /
#     pw2_residual / bias_gelu / edge_pad_1d), each with a CPU forward
#     as parity backstop.  All stride-parameterised so one compiled
#     kernel handles both [T, C] and [C, T] activations via op_params
#     layout flag.  GGML_OP_COUNT bumps 96 -> 101.  Consumed by
#     tts-cpp (qvac-ext-lib-whisper.cpp#15).
#   - Chatterbox Metal ops (diag_mask_inf, pad_ext lp0..lp3,
#     conv_transpose_1d simd_sum, mul_mv Q-variant bias/residual fusion,
#     gated behind compile-time flag).
#   - OpenCL persistent kernel binary cache; relaxed Adreno/Intel
#     device whitelist behind GGML_OPENCL_ALLOW_UNKNOWN_GPU.
#
# All changes are additive (new ops, new opt-in CMake flags); no API
# removals or op renames.  GGML_OP_COUNT bump and new ops only affect
# consumers that opt into them.
#
# Installed artefacts:
